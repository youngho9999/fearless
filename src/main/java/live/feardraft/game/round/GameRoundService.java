...
public class GameRoundService {
    private static final Map<String, BiConsumer<GameRound, String>> UPDATERS = new HashMap<>();
    ...
    private void updateGameRound(GameRound gameRound, PickClickRequest request) {
        BiConsumer<GameRound, String> updater = UPDATERS.get(request.getSequence());
        if (updater != null) {
            updater.accept(gameRound, request.getChampion());
        } else {
            throw new IllegalArgumentException("Invalid sequence: " + request.getSequence());
        }
    }
